...
def crear_tablero_con_naves(nivel="FACIL") -> list:
    """
    Crea un tablero de juego con naves colocadas aleatoriamente según el nivel de dificultad.

    Args:
        nivel (str, optional): El nivel de dificultad ("FACIL", "MEDIO", "DIFICIL"). Por defecto es "FACIL".

    Returns:
        list: El tablero de juego con las naves.
    """
    if nivel == "FACIL":
...
def manejar_disparo(tablero: list, tablero_disparos: list, posicion: int, dimension_pantalla: tuple) -> int:
    ...
    if 0 <= fila < len(tablero) and 0 <= columna < len(tablero[0]):
        if (
            ...
            else:  # Es una nave (valor_celda > 1)
